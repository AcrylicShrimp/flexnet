...
impl PrevotePayload {
    ...
    pub fn encode_signing_payload(&self, out: &mut Vec<u8>) {
        out.extend_from_slice(&MessageKind::Prevote.into_u8().to_le_bytes());
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct MsgPrevote {
    pub payload: PrevotePayload,
    pub signature: Signature,
}

impl MsgPrevote {
    pub fn new(payload: PrevotePayload, signature: Signature) -> Self {
...
impl MsgPrevote {
    ...
    pub fn decode_from(decoder: &mut Decoder) -> Result<Self, DecodeError> {
        ...
        Ok(Self::new(
            PrevotePayload::new(height, round, address, proposal_hash),
            signature,
        ))
    }
}
